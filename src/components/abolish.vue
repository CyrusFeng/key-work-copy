            ...
            <div>
                <div class="block">
                    <div class="title-wrap">
                        <field-title :blueline-visible="false" :redstar-visible="false">废止申请</field-title>
                    </div>
                    <div class="text">
                        <div class="radio">
                            <c-radio :options="options" @value-change="radioOnChange($event)"></c-radio>
                        </div>
                        <div v-show="isSubjective===1">
                            <c-textarea default-value=""
                                        remark=""
                                        tips="请输入主观原因"
                                        @update:inputValue="reason1 = $event"></c-textarea>
                        </div>
                        <div v-show="isSubjective===2">
                            <c-textarea default-value=""
                                        remark=""
                                        tips="请输入客观原因"
                                        @update:inputValue="reason2 = $event"></c-textarea>
                        </div>
                ...
                <div class="block approver" v-if="$store.state.taskType === 'charge' && $store.getters.currentRole !== 2 && $store.getters.currentRole !== 4">
                    <div class="title-wrap">
                        <div>
...
<script>
  ...
  export default {
    ...
    beforeRouteEnter(to, from, next) {
      qing.call('setWebViewTitle',{'title':'废止'})
      ...
      next((vm) => {
        ...
        // vm.isSign = vm.detailObj.signerID?true:false
        ...
        if (vm.detailObj.chargerID) {
          ...
          //   "eid": "8738001",
          //   "type": 1,
          //   "array": [vm.detailObj.chargerID]
          // }
          // let yunzhijiaURL = vm.$url + "/taskSystem/intf/cloud2"
          //
          // axios({
          //   method: 'post',
          //   url: yunzhijiaURL,
          ...
          //   "url":"/gateway/openimport/open/person/get",
          //   "authorize":"resGroupSecret",
          //   "dataStr":{"eid":"8738001","type":1,'array':[mine.openID]}
          // }
          // yunParams.dataStr = JSON.stringify(yunParams.dataStr);
          // $.ajax({
          //   url: urlTxt + '/intf/cloud2',
          //   dataType: 'json',
          //   data: yunParams,
          //   method: 'POST',
          ...
          // vm.getPersonInfoByOpenID(vm.detailObj.chargerID).then((response) => {
          //   alert('chargerID success')
          //   vm.leadersLeader = {
          //     'openId': response.data.result.data[0].openId,
          //     'name': response.data.result.data[0].name,
          //     'photoUrl': response.data.result.data[0].photoUrl,
    ...
    methods: {
      ...
      saveData() {
        ...
        taskLifeList[0].cancel_type = this.isSubjective
        if (this.isSubjective === 1) {
          taskLifeList[0].cancel_reason = this.reason1
        } else if (this.isSubjective === 2) {
          taskLifeList[0].cancel_reason = this.reason2
        }
